...
object IntelligenceEngine {
    ...
    private suspend fun analyzeRecentOnCpu(
        ...
        for (offset in 0 until maxDays) {
            ...
            // #195/#205: whole-night HRV cleaning-pipeline summary to the always-on strap log, so a
            // "reads ~2x too high" report is triageable without the HRV test mode: RMSSD vs SDNN
            // (rmssd >> sdnn = beat-to-beat jitter surviving the ectopic filter, not real HRV), meanNN
            // as an HR sanity-check, and how many R-R intervals survived cleaning (a low count also
            // flags the sparse-capture / calibration side — `nInput` is set before the min-beats gate,
            // so a sparse night still shows its count with rmssd=nil). A SEPARATE analyzeRaw pass over
            // the in-sleep R-R — does NOT touch the shipped windowed avgHrv. Byte-identical line format
            // to upstream 286e8b88 (Swift + Android twins).
            ...
            if (sleepRr.isNotEmpty()) {
                ...
                val rej = if (h.nInput > 0)
                    String.format(java.util.Locale.US, "%.0f", 100.0 * (1.0 - h.nClean.toDouble() / h.nInput))
                else "0"
                diag("hrv diag day=${res.daily.day} rmssd=${ms(h.rmssd)}ms sdnn=${ms(h.sdnn)}ms " +
        ...
        for (res in scoredNights) {
            ...
            for (s in res.workouts) {
                ...
                if (collider != null) {
                    // #510 (upstream 0bfbfc1e): the detected bout's own avgHR/calories/maxHR/strain come
                    // from the SAME motion+HR trace the detector used to find this activity's actual
                    // boundaries — often a tighter match than the colliding row's own [startTs,endTs]
                    // (e.g. a manual entry typed in afterward, whose guessed boundaries can clip most of
                    // the real HR-rich period and leave WhoopRepository.fillWorkoutHrFromStrap's raw
                    // window read too thin, silently showing no HR/calories). Backfill ONLY the fields
                    // the real row doesn't already have, then upsert in place (same natural key), rather
                    // than throwing the computed values away purely to avoid double-counting.
     ...
     */
    /**
     * #510 (upstream 0bfbfc1e): backfill ONLY the avgHr/maxHr/energyKcal/strain fields [real] doesn't
     * already have, from a detected bout's own computed values — never touching a field that's already
     * present, whether typed by the user, imported, or filled by an earlier pass. Returns [real]
     * unchanged (`==`) when it already had everything, so the caller can tell whether a write is
     * actually needed.
     */
