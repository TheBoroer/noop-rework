...
class SleepStagerOffWristTest {
    ...
    fun wornNightWithDenseHRStillRegisters() {
        ...
        assertEquals(1, sessions.size, "a worn night with dense, gap-free HR must still register")
    }
    ...
    fun realNightWithShortOffWristTailIsKept_hrGapPath() {
        ...
        val sessions = SleepStager.detectSleep(hr = hr, gravity = grav)
        assertEquals(1, sessions.size, "a real night with a short (<50%) off-wrist morning tail must be KEPT, not dropped")
    }
    ...
    fun realNightWithShortOffWristTailIsKept_intervalPath() {
        ...
        assertEquals(1, sessions.size, "a real night with a short (<50%) explicit off-wrist tail must be KEPT")
    }
    ...
    fun wristOffIntervalCoveringMostOfRunDropsIt() {
        ...
        assertTrue(dropped.isEmpty(), "a WRIST_OFF interval covering ≥50% of the run must drop it")
    }
    ...
    fun briefWristOffBlipKeepsWornNight() {
        ...
        assertEquals(1, kept.size, "a brief (<50%) WRIST_OFF blip must NOT drop a real worn night")
    }
