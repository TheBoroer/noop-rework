...
class IllnessDistanceTest {
    ...
    fun identityDistanceIsEuclideanNorm() {
        ...
        assertTrue(r.fires, "D > 2.5 and 2 deviating features -> fires")
    }
    ...
    fun correlationDiscountsSharedVariance() {
        ...
        assertTrue(correlated.distance < indep.distance, "correlated co-movement counts as one move, not two")
    }
    ...
    fun wellnessWardSignalDoesNotCountAsDeviating() {
        ...
        assertFalse(r.fires, "a big D driven by a wellness-ward coordinate must not fire")
    }
    ...
    fun singularCorrelationIsRegularizedToFinite() {
        ...
        assertTrue(r.distance < 4.2426, "collinear co-movement is one effective move, not two")
    }
    ...
    fun mahalanobisFireRateDoesNotBalloonOrSilence() {
        ...
        assertTrue(mCount >= lower, "Mahalanobis path must not SILENCE alerts vs the z-sum")
        assertTrue(mCount <= upper, "Mahalanobis path must not BALLOON alerts vs the z-sum")
        for (v in corpus) {
            assertEquals(zSumFires(v), mahaFires(v), "z-sum and Mahalanobis disagree on $v")
        }
    }
