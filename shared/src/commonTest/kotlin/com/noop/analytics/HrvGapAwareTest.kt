...
/**
 * Gap-aware RMSSD/pNN50, ported from upstream #204 (Android HrvGapAwareTest) / #208 (Swift twin).
 *
 ...
 * window and bias RMSSD high (the 2x over-read in upstream #195). These tests prove:
...
class HrvGapAwareTest {
    ...
    fun cleanSeriesIsUnchanged() {
        ...
        assertEquals(rr.size, clean.size, "fixture must have no drops")

        val cleaned = HrvAnalyzer.cleanRRGapAware(rr)
        assertEquals(clean, cleaned.nn)
        assertTrue(cleaned.contiguous.drop(1).all { it }, "no drops -> every survivor contiguous")
    ...
    fun midSeriesDropDoesNotSplice() {
        ...
        assertTrue(plainSpliced > 10.0, "plain RMSSD splices the 100 ms jump: $plainSpliced")
    }
    ...
    fun endDropsKeepAllContiguous() {
        ...
        assertTrue(cleaned.contiguous.drop(1).all { it }, "no interior gap")
