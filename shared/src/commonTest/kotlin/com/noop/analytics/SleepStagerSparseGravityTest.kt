...
class SleepStagerSparseGravityTest {
    ...
    fun sparseGravityNightNotShredded() {
        ...
        assertTrue(SleepStager.isGravitySparse(grav, hr), "clumped gravity must read as sparse")

        val sessions = SleepStager.detectSleep(hr = hr, gravity = grav)
        assertEquals(1, sessions.size, "a sparse-gravity night must be ONE session, not shredded")
        val s = sessions[0]
        // One ~6 h span (bounded by first/last gravity sample), not a sub-60-min fragment.
        assertTrue((s.end - s.start).toDouble() > 5.0 * 60 * 60, "the bridged session must be ~6 h, not a sub-hour fragment")
    ...
    fun denseGravityNightUnchangedBySparsePath() {
        ...
        assertFalse(SleepStager.isGravitySparse(grav, hr), "dense 1 Hz gravity must NOT read as sparse")
    ...
    fun buildRunsDenseGravityByteIdenticalToLegacy() {
        ...
        assertTrue(legacy.size >= 2, "a real >20-min gap must still split the dense path")
    }
    ...
    fun gravitySparseGateConditions() {
        ...
        assertTrue(SleepStager.isGravitySparse(clumped, hr), "short gravity span → sparse")
        ...
        assertTrue(SleepStager.isGravitySparse(bigGaps, hr), "a large inter-sample gap → sparse")

        // (c) Dense gravity over the same span is NOT sparse.
        val dense = stillGravity(start, 6 * 60 * 60)
        assertFalse(SleepStager.isGravitySparse(dense, hr), "dense gravity → not sparse")

        // (d) Degenerate HR (<2 samples) keeps the dense path regardless of gravity.
        assertFalse(SleepStager.isGravitySparse(bigGaps, emptyList()), "no HR span → keep dense path")
        ...
        assertTrue(SleepStager.isGravitySparse(clumpedBigGap, hr), "clumped gravity + one long dropout (small median, large max) → sparse")
    }
    ...
    fun clumpedGravityWithLongDropoutBridged() {
        ...
        assertTrue(SleepStager.isGravitySparse(grav, hr), "clumped motion with a long dropout (small median, large max gap) must read as sparse")
        val sessions = SleepStager.detectSleep(hr = hr, gravity = grav)
        assertEquals(1, sessions.size, "the dropout must be bridged into ONE session, not dropped sub-60-min fragments")
        assertTrue((sessions[0].end - sessions[0].start).toDouble() > (2 * block).toDouble(), "the bridged session must span both blocks across the dropout")
    }
}
