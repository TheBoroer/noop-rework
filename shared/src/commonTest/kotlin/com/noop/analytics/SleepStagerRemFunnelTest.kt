...
class SleepStagerRemFunnelTest {
    ...
    fun remRejectReasonAttributesEachGate() {
        ...
        assertEquals(SleepStager.REMRejectReason.REM_ELIGIBLE, reason(feature(0.0, 80.0, 5.0, 20.0, 2.0)), "still + cardiac + irregular resp → REM")

        // notStill: moving body, hr mid (60) + flat HR-variability → not cardiac, so NOT wake; the
        // REM rule fails first on stillness.
        assertEquals(SleepStager.REMRejectReason.NOT_STILL, reason(feature(0.5, 60.0, 0.0, 60.0, 2.0)), "moving body (no cardiac) → blocked notStill")

        // noCardiacActivation: still + irregular resp but HR mid + flat HR-variability.
        assertEquals(SleepStager.REMRejectReason.NO_CARDIAC_ACTIVATION, reason(feature(0.0, 60.0, 0.0, 20.0, 2.0)), "still + irregular resp but no cardiac → blocked")
        ...
        assertEquals(SleepStager.REMRejectReason.RESP_REGULAR, reason(feature(0.0, 80.0, 5.0, 20.0, 0.1)), "still + cardiac but regular resp → blocked respRegular")
        ...
        assertEquals(SleepStager.REMRejectReason.NO_RESP_FALLBACK_BAR, reason(feature(0.0, 80.0, 0.0, 20.0, Double.NaN)), "resp absent + no-resp bar unmet → blocked")
    }
    ...
    fun zeroRemNightSurfacesRespAbsent() {
        ...
        assertNotNull(diag)
        val d = diag!!
        assertTrue(d.isZeroREM, "a flat still low-HR no-resp night has 0% REM")
        assertEquals(0, d.remAfterReimpose)
        assertFalse(d.respChannelPresent, "no resp and no R-R → respChannelPresent false")
        assertTrue(d.sleepEpochs > 0, "the sleep period must contain epochs to explain")
        ...
        assertEquals(d.sleepEpochs, attributed, "per-epoch reasons must partition the sleep epochs")
        assertTrue(d.summary.contains("resp=ABSENT"), "summary surfaces the absent resp channel")
    }
    ...
    fun diagnosticIsReadOnly() {
        ...
        assertEquals(before, after, "remFunnelDiagnostic must not change the staged hypnogram")
    }
    ...
    fun sparseCardiacDoesNotPromoteStillSleepToWakeOnHrVarAlone_705() {
        ...
        val dense = SleepStager.classifyOne(ppgWakeEpoch(),
            ...
            cardiacSparse = false)
        assertEquals("wake", dense, "dense 4.0 night keeps the full hrHigh||hrvarHigh wake signal")
        ...
        val sparse = SleepStager.classifyOne(ppgWakeEpoch(),
            hrLo = 55.0, hrHi = 90.0, rmssdHi = 50.0, hrvarHi = 100.0, rrvHi = 1.0, rrvLo = 0.5,
            cardiacSparse = true)
        assertNotEquals("wake", sparse, "a sparse/PPG night must not flip still low-HR sleep to wake on hrVar alone")
    }
    ...
    fun cardiacSparseFlagFiresOnMostlyMissingRmssd_705() {
        ...
        assertTrue(SleepStager.isCardiacSparse(listOf(noRR, noRR, noRR, withRR)), "3/4 epochs missing R-R is sparse-cardiac")
        assertFalse(SleepStager.isCardiacSparse(listOf(withRR, withRR, withRR, noRR)), "1/4 epochs missing R-R is a dense (4.0-style) night")
        assertFalse(SleepStager.isCardiacSparse(emptyList()), "empty session is not sparse")
    }
    ...
    fun stillPpgNightNoLongerScoresMostlyWake_705() {
        ...
        assertTrue(wakeShare(false) > 0.80, "dense rule still over-reports wake on a noisy-hrVar night (reproduces #705)")
        // Sparse-cardiac gate: only the real HR-elevated awakenings stay wake (~10%).
        assertTrue(wakeShare(true) < 0.40, "a still PPG night must not be classified as mostly wake (was 40%+ before #705)")
    }
}
