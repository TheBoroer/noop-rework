 ...
 * genuinely elevated (awake) run is still rejected. Ported from upstream #268 (3b168690).
 */
class SleepStagerHrConfirmTest {
    ...
    fun spikyButAsleepRunSurvivesWhereMeanWouldDropIt() {
        ...
        assertTrue(mean > bar, "fixture: mean ($mean) must exceed bar ($bar)")
        assertTrue(median <= bar, "fixture: median ($median) must be at/under bar ($bar)")
