...
class SleepStageHealTest {
    ...
    fun editedBeforeSyncHealsToRealStagesOnceRawArrives() {
        ...
        assertNotNull(real, "dense raw over the locked window must re-derive real stages")
        assertNotEquals(fabricated, real, "real stages must differ from the fabricated reclip block")
        ...
        assertTrue(real!!.trimStart().startsWith("["), "re-derived JSON must be a segment array")
        val realSegments = SleepStageTotals.minutes(real)
        assertNotNull(realSegments, "real stages must decode to stage minutes")
        assertTrue(realSegments!!.asleep > 0.0, "a 6h dense night must record real asleep time, not all-wake")
    }
    ...
    fun noRawNightStaysAsEdited() {
        ...
        assertFalse(SleepStageHealer.isDense(sparseGrav, start, end), "5 samples over a 6 h window must NOT clear the density gate")

        val derived = SleepStageHealer.restageFromSamples(start, end, sparseGrav, emptyList(), emptyList(), emptyList())
        assertNull(derived, "a non-dense night must return null so the stored stages are kept")
    }
    ...
    fun secondPassIsIdempotent() {
        ...
        assertNotNull(first)
        assertEquals(first, second, "re-deriving over identical bounds+raw must be byte-identical (equality-skip)")
    }
    ...
    fun endToEndHealsThenIsIdempotent() {
        ...
        assertEquals(1, w1, "the edited-before-sync night must heal exactly once")
        val healed = afterFirst.single()
        assertEquals(start, healed.startTs, "bed bound must be untouched")
        assertEquals(end, healed.endTs, "wake bound must be untouched")
        assertTrue(healed.userEdited, "userEdited must stay set")
        assertNotEquals(fabricated, healed.stagesJSON, "stages must have been replaced")
        ...
        assertEquals(0, w2, "a second pass must be a no-op")
    }
    ...
    fun endToEndNoRawLeavesEditUntouched() {
        ...
        assertEquals(0, writes, "a no-raw night must not be written")
        assertEquals(fabricated, rows.single().stagesJSON, "the user's edited (fabricated) stages must remain")
    }
    ...
    fun encodeStagesIsStableAcrossCalls() {
        ...
        assertEquals(a, b, "identical segments must encode byte-identically every call")
    }
}
