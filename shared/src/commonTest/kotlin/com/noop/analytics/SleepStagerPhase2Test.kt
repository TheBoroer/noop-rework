...
class SleepStagerPhase2Test {
    ...
    fun detectSleepClampsOverlongBadClockBlock() {
        ...
        assertTrue(SleepStager.detectSleep(hr = hr, gravity = grav).isEmpty(), "an 18 h still block is a bad-clock artefact and is dropped by the span cap")
    }
    ...
    fun detectSleepKeepsLongButPlausibleNight() {
        ...
        val grav = stillGravity(start, dur)
        val hr = hrStream(start, dur, 50)
        assertEquals(1, SleepStager.detectSleep(hr = hr, gravity = grav).size, "a 15 h night is below the cap and survives")
    }
    ...
    fun morningStillnessRejectedNearOvernightWake() {
        ...
        assertFalse(SleepStager.passesMorningStillnessGuard(p, 74, 80.0, wakeEnd), "a still block right after the overnight wake with no clear re-onset dip is rejected")
    }
    ...
    fun morningStillnessKeptOnStrongReonsetDip() {
        ...
        val p = daytimePeriod(9, 120)
        val wakeEnd = startAtHour(8)
        assertTrue(SleepStager.passesMorningStillnessGuard(p, 70, 78.0, wakeEnd), "a clear re-onset HR dip keeps a genuine morning second sleep")
    }
    ...
    fun morningStillnessGuardNoOpOutsideWindow() {
        ...
        assertTrue(SleepStager.passesMorningStillnessGuard(p, 70, 80.0, null), "outside the morning window the guard is the ordinary daytime bar")
    }
    ...
    fun morningStillnessRescuedByBandSleepState() {
        ...
        assertTrue(SleepStager.passesMorningStillnessGuard(p, 74, 80.0, wakeEnd, band), "the strap's own 'asleep' band rescues a borderline-HR morning re-onset")
        assertFalse(SleepStager.passesMorningStillnessGuard(p, 74, 80.0, wakeEnd), "without the band anchor the same borderline block is rejected")
    }
    ...
    fun bandStateConfirmsAsleepFractionGate() {
        ...
        assertFalse(SleepStager.bandStateConfirmsAsleep(p, emptyList()), "empty band → never confirmed (no fabricated reading)")
    }
    ...
    fun sessionEpochMotionGridsToStageEpochs() {
        ...
        assertEquals(180, motion.size, "one motion value per 30 s epoch")
        assertTrue(motion.all { it >= 0.0 }, "motion magnitudes are non-negative |Δgravity| sums")
        assertEquals(0.0, motion.sum(), 1e-6, "a perfectly still stream has ~zero motion")
    }

    @Test
    fun sessionEpochMotionEmptyWhenNoGravity() {
        assertTrue(SleepStager.sessionEpochMotion(0L, 1800L, emptyList()).isEmpty(), "too little gravity to grid → [] so the caller persists NULL")
    }
    ...
    fun sessionEpochSleepStateGridsOnePerEpoch() {
        ...
        assertEquals(180, states.size, "one band-state value per 30 s epoch")
        assertTrue(states.all { it == 2 }, "an all-asleep band grids to all-asleep epochs")
    }
    ...
    fun sessionEpochSleepStateEmptyWhenNoBandSamples() {
        ...
        assertTrue(SleepStager.sessionEpochSleepState(0L, 1800L, listOf(9_000L to 2)).isEmpty(), "samples outside the window are ignored")
    }
    ...
    fun sessionGridFeedsTheReonsetGuardEndToEnd() {
        ...
        assertTrue(SleepStager.passesMorningStillnessGuard(p, 74, 80.0, wakeEnd, reconstructed), "the persisted+re-expanded band grid drives the H7 confirm end to end")
    }
}
