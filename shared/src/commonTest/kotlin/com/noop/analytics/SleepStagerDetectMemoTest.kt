...
class SleepStagerDetectMemoTest {
    ...
    fun identicalInputsTwiceReturnIdenticalSessions() {
        ...
        assertTrue(first.isNotEmpty(), "the synthetic night must actually detect")
    ...
    fun gravityAxesAreKeyedSeparatelyNotAsASum() {
        ...
        assertTrue(a.isNotEmpty(), "the still night must detect")
        assertTrue(
            SleepStager.detectSleep(hr = hr, gravity = flippingB).isEmpty(),
            "the axis-flipping stream is motion, not sleep — a non-empty result here means the " +
