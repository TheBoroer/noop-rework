...
class SleepStagerDaytimeGuardTest {
    ...
    fun daytimeShortLowHRWindowRejected() {
        ...
        assertTrue(sessions.isEmpty(), "a 70-min daytime still window must be rejected by the guard")
    }
    ...
    fun daytimeQualityNapRegisters() {
        ...
        val sessions = SleepStager.detectSleep(hr = dayHR + napHR, gravity = dayGrav + napGrav)
        assertEquals(1, sessions.size, "a 120-min daytime nap with a real HR dip must register")
    ...
    fun overnightShortWindowUnchanged() {
        ...
        assertEquals(1, sessions.size, "a 70-min overnight still window must register unchanged")
    ...
    fun tzOffsetShiftsWindowIntoDaytimeBand() {
        ...
        assertTrue(shifted.isEmpty(), "a +10h offset pushes the window into the daytime band → rejected")
    }
