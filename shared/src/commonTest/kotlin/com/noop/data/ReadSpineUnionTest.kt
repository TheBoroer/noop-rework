...
class ReadSpineUnionTest {
    ...
    fun canonicalImportHistorySurvivesAfterReAdd() {
        ...
        assertEquals(listOf("2026-06-10", "2026-06-11"), merged.map { it.day }, "both imported days must survive the re-add, not be orphaned")
    ...
    fun activeLiveRowWinsADayBothIdsCover() {
        ...
        assertEquals(1, union.count { it.day == day }, "the day is not double-counted")
        assertEquals(480.0, union.first { it.day == day }.totalSleepMin, "the active/live row wins the day")
    }
