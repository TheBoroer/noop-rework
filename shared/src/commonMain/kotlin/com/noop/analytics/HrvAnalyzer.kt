...
object HrvAnalyzer {
    ...
    //
    // Cleaning drops out-of-range and ectopic beats; a successive-difference metric run over the
    // COMPACTED clean list would then count the difference ACROSS each splice as a real beat-to-beat
    // delta. Because RMSSD squares each delta, one removed beat can dominate the window and bias
    // RMSSD high (the 2x over-read in upstream #195/#204). [cleanRRGapAware] cleans while remembering
    // where beats were dropped, and [rmssdGapAware]/[pnn50GapAware] skip any difference that straddles
    // a gap. On a series with no drops the results are identical to the plain estimators.

    /**
     * A cleaned NN series that remembers where beats were dropped. `contiguous[i]` is true when `nn[i]`
     * and `nn[i - 1]` were adjacent beats in the ORIGINAL series (no beat removed between them by the
     * range or ectopic filter) and false when a beat was dropped in between (a splice). Index 0 is
     * always false: the first beat has no predecessor.
     */
    ...
    fun analyzeRaw(rawRR: List<Double>, maxRejectedFraction: Double? = null): HrvResult {
        ...
        // spurious successive difference (#195/#204). SDNN and meanNN use no successive differences
        // and read the same clean beats as before.
        val rmssd = rmssdGapAware(clean, cleaned.contiguous)
        val sdnn = sdnnRaw(clean)
        val mean = clean.sum() / clean.size.toDouble()
        val pnn50 = pnn50GapAware(clean, cleaned.contiguous)
