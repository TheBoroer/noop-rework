...
object ReadinessEngine {
    ...
    /** Access-order LRU (cap [EVALUATE_CACHE_CAP]): a multiplatform [MpLru] (Task 1), identical
     *  bounded evict-eldest behavior to the JVM `LinkedHashMap(cap, 0.75f, accessOrder = true)` +
     *  manual-eviction pair it replaces (Phase 2a Task 6 hoist), locking internal to the container.
     *  Twin of Swift's AnalyticsMemoCache(capacity: 16). */
    private val evaluateCache = MpLru<ReadinessKey, Readiness>(EVALUATE_CACHE_CAP)
     ...
     * the 0-decimal case uses `roundToLong` (ties toward positive infinity, exactly `Math.round`,
     * matching Swift `Int(x.rounded())` on the values this formats), NOT printf's "%.0f"; the >0
     * case uses [toFixed] (Task 2), the locale-independent replacement for
     * `String.format(Locale.US, "%.Nf", x)` with tie-exact JVM parity, so the separator is always ".".
     */
    private fun fmt(x: Double, decimals: Int): String =
        if (decimals == 0) x.roundToLong().toString()
        else x.toFixed(decimals)
