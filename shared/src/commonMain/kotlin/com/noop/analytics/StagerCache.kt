...
internal object StagerCache {
     ...
     *  (Phase 2a Task 6 hoist), with the locking internal to the container. Only the cache plumbing
     *  changed; a hit/miss/eviction sequence and the staged output are byte-identical. */
    private val cache = MpLru<Key, List<StageSegment>>(MAX_NIGHTS)

    fun get(key: Key): List<StageSegment>? = cache.get(key)
