...
object HydrationStore {
     ...
     *  (Task 6: the kotlinx-datetime mapping of java.util.TimeZone.getDefault().getOffset(atMillis),
     *  the Task 3 conversion this file's tag pre-verified; pinned by AnalyticsFormatPinningTest.) */
    private fun localOffsetSec(atMillis: Long = Clock.System.now().toEpochMilliseconds()): Long =
        TimeZone.currentSystemDefault()
            .offsetAt(Instant.fromEpochMilliseconds(atMillis)).totalSeconds.toLong()

    /** The LOCAL yyyy-MM-dd day key for a unix-seconds [ts] (defaults to now). */
    fun dayKey(ts: Long = Clock.System.now().epochSeconds): String =
     ...
     */
    suspend fun log(repo: WhoopRepository, amountMl: Int, ts: Long = Clock.System.now().epochSeconds): Double {
     ...
     */
    suspend fun set(repo: WhoopRepository, totalMl: Double, ts: Long = Clock.System.now().epochSeconds): Double {
        val day = dayKey(ts)
     ...
     */
    suspend fun remove(repo: WhoopRepository, amountMl: Int, ts: Long = Clock.System.now().epochSeconds): Double {
        if (amountMl <= 0) return total(repo, ts)
    ...
    suspend fun total(repo: WhoopRepository, ts: Long = Clock.System.now().epochSeconds): Double {
        val day = dayKey(ts)
