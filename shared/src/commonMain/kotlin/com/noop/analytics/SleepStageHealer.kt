...
/** Read cap per stream read, matching the Swift 200_000 bound. Moved here from
 *  IntelligenceEngine's STREAM_LIMIT member in Task 6: this healer hoisted to commonMain while
 *  IntelligenceEngine stays androidMain (Baselines/SharedPreferences boundary), and both (plus
 *  IntelligenceEngine's own read loops) share this ONE definition (same package, so the engine's
 *  unqualified references resolve here). */
...
object SleepStageHealer {
    ...
    suspend fun restageFromRaw(
        ...
        val lo = start - 3_600L
        val hi = end + 3_600L
        val grav = repo.gravitySamples(deviceId, lo, hi, STREAM_LIMIT)
        ...
        val hr = repo.hrSamples(deviceId, lo, hi, STREAM_LIMIT)
        val rr = repo.rrIntervals(deviceId, lo, hi, STREAM_LIMIT)
        val resp = repo.respSamples(deviceId, lo, hi, STREAM_LIMIT)
