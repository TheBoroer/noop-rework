...
object SleepStager {
    ...
    internal fun confirmSleepWithHR(p: Period, hr: List<HrSample>, baseline: Double?): Boolean {
        ...
        // Ported from upstream #268 (3b168690).
    ...
    /** Multiplatform access-order LRU ([MpLru], Task 1): identical bounded evict-eldest behavior to
     *  the JVM `LinkedHashMap(capacity, 0.75f, accessOrder = true)` + manual-eviction pair it replaces
     *  (Phase 2a Task 6 hoist), with the locking internal to the container. The detection/staging math
     *  is untouched; a hit is still byte-identical to recomputing. */
    private val detectCache = MpLru<DetectKey, List<DetectedSleep>>(DETECT_CACHE_MAX_DAYS)
    ...
    fun detectSleep(
        ...
        detectCache.get(key)?.let { return copyDetected(it) }
        // Compute OUTSIDE the lock (the Swift AnalyticsMemoCache does the same; MpLru locks only per
        // get/put): a slow night never serialises another thread's lookup, and a racing duplicate
        // compute just overwrites the entry with an identical value — benign, the function is
        // deterministic.
        val sessions = detectSleepUncached(hr, rr, resp, gravity, tzOffsetSeconds, wristOff,
            bandSleepState, useSleepStagerV2, null)
        detectCache.put(key, copyDetected(sessions))
        return sessions
    }
    ...
    internal fun sessionHrvWindows(
        ...
        while (t < end) {
            ...
            // remove. See HrvAnalyzer.rmssdGapAware. (#195/#204)
