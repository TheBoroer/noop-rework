 ...
 *
 * The transaction boundary is injected as [transactor] (defaulting to Room's KMP write transaction:
 * `useWriterConnection { it.immediateTransaction { ... } }`, the common-artifact replacement for the
 * Android-only `androidx.room.withTransaction` extension; BEGIN IMMEDIATE + COMMIT/ROLLBACK, so the
 * wrapped block stays all-or-nothing) so the registry's logic is exercisable on the plain JVM without
 * a real Room database, mirroring how the rest of the test suite stays Robolectric-free (see
 * DeviceRegistryTest / MoodStoreTest).
 */
class DeviceRegistry(
     ...
     *  `immediateTransaction`; tests pass through.
    ...
    /** Production constructor: wraps the DAO + Room's KMP write transaction over [db]. */
     ...
     */
    suspend fun setActive(id: String, now: Long = Clock.System.now().epochSeconds) {
        transactor.run {
            dao.demoteActive()
            dao.promote(id, now)
        }
