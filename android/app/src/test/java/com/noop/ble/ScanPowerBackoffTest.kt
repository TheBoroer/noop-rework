 ...
 * crosses [AndroidWhoopBleClient.SCAN_POWER_BACKOFF_THRESHOLD] — so an out-of-range strap (left at home / dead
...
class ScanPowerBackoffTest {
    ...
    @Test fun zeroAttemptsIsLowLatency() {
        ...
        assertEquals(
            ...
            AndroidWhoopBleClient.scanModeForReconnectAttempts(0),
        )
    }

    @Test fun belowThresholdStaysLowLatency() {
        for (n in 1 until AndroidWhoopBleClient.SCAN_POWER_BACKOFF_THRESHOLD) {
            assertEquals(
                "attempt $n should still be LOW_LATENCY",
                ScanSettings.SCAN_MODE_LOW_LATENCY,
                AndroidWhoopBleClient.scanModeForReconnectAttempts(n),
            )
        }
    }
    ...
    @Test fun atThresholdBacksOffToBalanced() {
        assertEquals(
            ...
            AndroidWhoopBleClient.scanModeForReconnectAttempts(AndroidWhoopBleClient.SCAN_POWER_BACKOFF_THRESHOLD),
        )
    }
    ...
    @Test fun pastThresholdStaysBalanced() {
        assertEquals(
            ScanSettings.SCAN_MODE_BALANCED,
            AndroidWhoopBleClient.scanModeForReconnectAttempts(AndroidWhoopBleClient.SCAN_POWER_BACKOFF_THRESHOLD + 20),
        )
    }
}
