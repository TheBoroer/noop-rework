...
/**
 * Pins the #78 hole-4 salvage-probe gate ([AndroidWhoopBleClient.shouldSalvageProbe]): the one bounded
 ...
 * per [AndroidWhoopBleClient.BOND_LOOP_SALVAGE_FLOOR_MS] window, and the give-up stays latched throughout).
 * Twin of the Swift `BondLoopHardeningTests` probe cases.
 */
class BondLoopSalvageProbeTest {

    private val floor = AndroidWhoopBleClient.BOND_LOOP_SALVAGE_FLOOR_MS

    @Test
    fun firesPastFloorWhilePaused() {
        assertTrue(AndroidWhoopBleClient.shouldSalvageProbe(
    ...
    fun respectsTheFloor() {
        ...
        assertFalse(AndroidWhoopBleClient.shouldSalvageProbe(
            pausedForBondLoop = true, connected = false,
