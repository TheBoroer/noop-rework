...
/**
 * The "Sync now" gate (#93). The manual button forwards to the SAME [AndroidWhoopBleClient.requestSync]
 ...
 * ([AndroidWhoopBleClient.canRequestSync]) so the no-op behaviour the button relies on can't silently
...
class SyncNowGateTest {
    ...
    fun allowsSync_whenConnectedAndBondedAndIdle() {
        assertTrue(AndroidWhoopBleClient.canRequestSync(connected = true, bonded = true, backfilling = false))
    }

    @Test
    fun blocksSync_whenNotConnected() {
        // Disconnected: the command channel is gone — a kick would just be dropped by send().
        assertFalse(AndroidWhoopBleClient.canRequestSync(connected = false, bonded = true, backfilling = false))
    }
    ...
    fun blocksSync_whenNotBonded() {
        ...
        assertFalse(AndroidWhoopBleClient.canRequestSync(connected = true, bonded = false, backfilling = false))
    }
    ...
    fun blocksSync_whenAlreadyBackfilling() {
        ...
        assertFalse(AndroidWhoopBleClient.canRequestSync(connected = true, bonded = true, backfilling = true))
    }

    @Test
    fun blocksSync_whenDisconnectedMidBackfill() {
        // A dropout mid-backfill: every precondition that matters is false — still a no-op.
        assertFalse(AndroidWhoopBleClient.canRequestSync(connected = false, bonded = false, backfilling = true))
    }
}
