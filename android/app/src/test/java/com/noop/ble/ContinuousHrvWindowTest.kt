...
/**
 * Pins the #927 Continuous HRV "overnight only" window predicate ([AndroidWhoopBleClient.overnightWindowContains]
 * + [AndroidWhoopBleClient.continuousHrvStreamWanted]).
 *
...
class ContinuousHrvWindowTest {
    ...
    fun offMode_neverWants() {
        for (minute in listOf(0, 6 * 60, 12 * 60, 22 * 60, 1439)) {
            assertFalse(AndroidWhoopBleClient.continuousHrvStreamWanted(false, false, minute, start, end))
            assertFalse(AndroidWhoopBleClient.continuousHrvStreamWanted(false, true, minute, start, end))
        }
    }
    ...
    fun alwaysMode_wantsAllDay() {
        for (minute in listOf(0, 3 * 60, 6 * 60 + 59, 7 * 60, 12 * 60, 21 * 60 + 59, 22 * 60, 1439)) {
            assertTrue(AndroidWhoopBleClient.continuousHrvStreamWanted(true, false, minute, start, end))
        }
    }
    ...
    fun overnightMode_gatesOnWindow() {
        assertTrue(AndroidWhoopBleClient.continuousHrvStreamWanted(true, true, 23 * 60, start, end))    // 23:00 in
        assertFalse(AndroidWhoopBleClient.continuousHrvStreamWanted(true, true, 12 * 60, start, end))   // noon out
    }
    ...
    fun startBoundary_inclusive() {
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(22 * 60, start, end))
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(21 * 60 + 59, start, end))
    }

    /** Exclusive end: 07:00 exactly is OUTSIDE; 06:59 is inside. Matches quiet hours (`now < end`). */
    @Test
    fun endBoundary_exclusive() {
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(7 * 60, start, end))
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(6 * 60 + 59, start, end))
    }
    ...
    fun wrapAcrossMidnight() {
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(23 * 60 + 59, start, end))
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(0, start, end))
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(3 * 60, start, end))
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(12 * 60, start, end))
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(15 * 60, start, end))
    }
    ...
    fun nonWrappingWindow() {
        val s = 1 * 60
        val e = 5 * 60
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(0, s, e))
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(59, s, e))
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(60, s, e))          // 01:00 in
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(4 * 60 + 59, s, e))
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(5 * 60, s, e))     // 05:00 out
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(12 * 60, s, e))
    }
    ...
    fun degenerateEqualWindow_isEmpty() {
        ...
        for (minute in listOf(0, s - 1, s, s + 1, 23 * 60)) {
            assertFalse(AndroidWhoopBleClient.overnightWindowContains(minute, s, s))
        }
    }
    ...
    fun minuteDomainExtremes() {
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(0, start, end))       // 00:00 in
        assertTrue(AndroidWhoopBleClient.overnightWindowContains(1439, start, end))    // 23:59 in
        // And against a daytime window both extremes are out.
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(0, 9 * 60, 17 * 60))
        assertFalse(AndroidWhoopBleClient.overnightWindowContains(1439, 9 * 60, 17 * 60))
    }
}
