...
/**
 * Android background-survival helpers (#386, upstream 05c2d867). NOOP already runs a foreground
 * service + exact alarms, but aggressive OEM battery managers kill even those, so the reliable lever
 * is a USER action: whitelist NOOP from battery optimisation (and, on the worst vendors, enable
 * auto-start). This centralises the detection and the intents that fix it, so the Settings
 * "Keep NOOP alive overnight" toggle and the Test Centre diagnostics share ONE source of truth for
 * the vendor set + exempt check.
 *
 ...
 * The real cost is the existing "Keep connected in the background" toggle; this only makes the
 * overnight work the user already enabled actually survive the night.
 *
 * Android-only — no iOS analogue (Apple doesn't kill background apps this way).
 */
object BackgroundHealth {

    /**
     * Manufacturers whose proprietary battery managers kill background work regardless of the AOSP
     * foreground-service contract (the dontkillmyapp.com set). ONE canonical list —
     * [com.noop.testcentre.AndroidDiagnostics] and the Settings toggle both read it here so the two
     * can never drift. Pure.
     */
     ...
     * with [android.content.pm.PackageManager.resolveActivity] and the first that resolves wins; null →
     * the caller uses the generic battery screen. This is a SECOND, separate action (never chained onto
     * the exemption dialog), so a single user tap never spawns two popups. Never throws.
     */
