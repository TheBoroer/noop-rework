...
/**
 * What prompted a historical-offload attempt. Mirror of the upstream Swift `BackfillTrigger`
 * (Strand/BLE/BackfillPolicy.swift): the same six cases so [BackfillPolicy.shouldRun] gates
 * byte-identically across platforms (the cross-platform parity contract). (#228, upstream 0c552aaf)
 */
 ...
 * (observed: ~15-min periodic + expedited event syncs). Exact port of the upstream `BackfillPolicy`
 * — the constants, the empty-streak backoff curve, and the per-trigger rules are kept byte-identical
 * so Android and macOS throttle the same way. Ports the gate that [AndroidWhoopBleClient.requestSync]
 * previously flagged as unported ("requestSync minus the BackfillPolicy rate-limiter"), so a strap
 * that keeps offloading nothing (off-wrist / not banking) stops being re-polled every 15 min,
 * waking its NAND and holding the link. (#228)
 */
