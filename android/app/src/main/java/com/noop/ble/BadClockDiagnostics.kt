 ...
 * whose RTC reset to a wrong base (future- or far-past-dated banking). Port of upstream 694ae229's
 * `BadClockDiagnostics` (byte-identical output). Upstream keeps this in its protocol layer for Swift
 * parity; rework's protocol layer is KMP commonMain (no `java.text`), and the only consumer is
 * [AndroidBackfiller]'s log line, so it lives here. No state, no `now` inside — callers inject `now`
 * so the output is deterministic and unit-testable.
 */
